...
function handleApiGet_(e) {
  try {
    ...
    // Company endpoint (get company data including SMS opt-in status)
    if (endpoint === 'company' || endpoint === 'api/company') {
      const token = e.parameter && e.parameter.token;
      const companyNameParam = e.parameter && e.parameter.companyName;
      const companyName = companyNameParam || companyFromToken_(token);
      
      if (!companyName) {
        return jsonResponse_({ error: 'Company name or token is required' }, '', 400);
      }
      
      const companyData = getCompanyData_(companyName);
  ...
  try {
    ...
    if (endpoint === 'users' || endpoint === 'api/users') {
      ...
      if (String(methodOverride).toUpperCase() === 'PATCH') {
        ...
        // Handle SMS opt-in toggle (updates company's Enable_SMS_Notifications)
        if (body.smsOptIn !== undefined) {
          updateCompanySmsOptIn_(companyName, body.smsOptIn);
        }
