(function(){
  ...
  function clearServiceGrid() {
    ...
    grid.innerHTML = '<div class="no-services-message"><p>Enter property area to view available services</p></div>';
  ...
  function renderServiceGrid() {
    ...
    if (filteredServices.length === 0) {
      grid.innerHTML = '<div class="no-services-message"><p>No services match your search</p></div>';
      return;
    }
    
    grid.innerHTML = filteredServices.map(service => `
      <div class="service-card ${selectedService && selectedService.name === service.name ? 'selected' : ''}" 
           data-service-name="${sanitize(service.name)}"
           data-service-sku="${sanitize(service.sku)}">
        <div class="service-card-header">
          <div class="service-card-name">${sanitize(service.name)}</div>
          <div class="service-card-check">${selectedService && selectedService.name === service.name ? '✓' : ''}</div>
        </div>
        <div class="service-card-pricing">
          <div>
            <div class="service-card-price-label">Initial</div>
            <div class="service-card-price">$${service.initialPrice.toFixed(2)}</div>
          </div>
          <div>
            <div class="service-card-price-label">Recurring</div>
            <div class="service-card-price">$${service.recurringPrice.toFixed(2)}</div>
          </div>
        </div>
      </div>
    `).join('');
  ...
  function showSelectedService() {
    ...
    $('#selectedInitialPrice').textContent = `$${selectedService.initialPrice.toFixed(2)}`;
    $('#selectedRecurringPrice').textContent = `$${selectedService.recurringPrice.toFixed(2)}`;
  ...
  function updateServiceCount() {
    ...
    $('#serviceCount').textContent = `${count} service${count !== 1 ? 's' : ''}`;
  }
  ...
  function validate() {
    ...
    const areaValue = Number($('#areaInput').value) || 0;
    if (areaValue <= 0) {
      errors.squareFootage = 'Required for selected service';
    }
  ...
  async function loadConfig() {
    ...
    if (!companyToken) {
      throw new Error('No company token provided. Please use the correct intake form URL with your access token.');
    }
  ...
  function setupCompanyMode(company, products) {
    ...
    $('#companySubtitle').textContent = 'Agent Intake System';
